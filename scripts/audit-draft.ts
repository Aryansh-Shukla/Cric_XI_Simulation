...
for (const mode of modes) {
  ...
  for (let r = 0; r < runs; r++) {
    ...
    for (let round = 0; round < 11; round++) {
      ...
      let cands = PlayerEligibilityService.filterEligible(squad.players, picked).filter(
        (p) => canPickPlayer(p, { picked, mode, remainingSlots: slots }).canPick,
      );
      if (!cands.length) {
        cands = PlayerEligibilityService.filterEligible(
          pool.flatMap((s) => s.players),
          picked,
    ...
    if (picked.length === 11) {
      ...
      if (mode === "FRANCHISE_T20" && picked.filter((p) => p.isOverseas).length > 4)
        overseasBreach++;
    }
  }
  console.log(
    `${mode}: runs=${runs} deadEnds=${deadEnds} invalidXI=${invalidXI} duplicatePlayers=${dupes} overseasBreaches=${overseasBreach}`,
  );
}
