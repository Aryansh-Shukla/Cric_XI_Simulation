import { createTournament, advanceTournament, standingsTable, netRunRate, topRunScorers, topWicketTakers } from "@/lib/cricket/tournament";
...
for (const mode of modes) {
  let ok=0,err=0,fx=0,done=0,ai=0,titles=0,elim=0,goldBat=0,goldBowl=0,badNrr=0,badPts=0;
  for (let run = 0; run < 25; run++) {
    try {
      ...
      let g=0; while (!st.complete && g++ < 60) st = advanceTournament(st);
      if (!st.complete) throw new Error("incomplete");
      done += st.results.length; ai += st.aiResults.length;
      if (st.championshipWon) titles++; if (st.eliminated) elim++;
  ...
  console.log(`${mode}: ok=${ok} err=${err} fixtures=${fx} userMatches=${done} aiMatches=${ai} titles=${titles} eliminated=${elim} ourBatterInTop=${goldBat} ourBowlerInTop=${goldBowl} badPts=${badPts} badNRR=${badNrr}`);
}
