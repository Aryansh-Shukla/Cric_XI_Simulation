...
let mismatch = 0; const examples: string[] = [];
for (const s of SQUAD_REGISTRY) {
  for (const pid of s.playerProfileIds) {
    const p = PROFILE_REGISTRY.get(pid);
    if (!p) { examples.push(`MISSING ${pid}`); mismatch++; continue; }
    if (p.squadId !== s.id) { mismatch++; if (examples.length < 10) examples.push(`${s.id} label=${s.label} -> ${p.name} belongs ${p.squadId}`); }
  }
}
console.log("profile mismatches:", mismatch); console.log(examples);
