...
function pitchFlavor(pitch: Pitch): string[] {
  switch (pitch) {
    case "Green":
      return ["seam movement off the deck", "the ball nipping around", "green tinge doing plenty"];
    case "Flat":
      return [
        "a road of a surface",
        "batters licking their lips at the flat deck",
        "nothing in it for the bowlers",
      ];
    case "Dusty":
      return [
        "dust flying off the crumbling surface",
        "grip and turn from ball one",
        "a dry, tired pitch",
      ];
    case "Turning":
      return [
        "square turn for the spinners",
        "the ball ragging out of the rough",
        "a genuine bunsen",
      ];
    case "Slow":
      return [
        "the ball sticking in the surface",
        "a slow, low deck",
        "batters struggling for timing",
      ];
  }
}
function weatherFlavor(w: Weather): string {
  switch (w) {
    case "Sunny":
      return "under a blazing sun";
    case "Cloudy":
      return "under grey, brooding skies";
    case "Humid":
      return "in muggy, sticky air";
    case "Night Match":
...
export function limitedCommentary(r: LimitedScorecard, rng: () => number): string[] {
  ...
  if (ppRate > (format === "T20" ? 9 : 6.5)) {
    lines.push(
      `${firstBatting.teamName} tore into the powerplay — ${Math.round(ppRate * ppLength)} off the first ${ppLength} looked ominous.`,
    );
  } else if (ppRate < (format === "T20" ? 6 : 4)) {
    lines.push(
      `A watchful powerplay from ${firstBatting.teamName} — the new ball did enough to keep the scoring in check.`,
    );
  } else {
    lines.push(
      `${firstBatting.teamName} settled in through the powerplay without giving too much away.`,
    );
  }
  ...
  if (top1.runs >= 100) {
    lines.push(
      `${top1.name} raised a stunning hundred — ${top1.runs} off ${top1.balls}, the innings built around him.`,
    );
  } else if (top1.runs >= 75 && sr1 > 140) {
    lines.push(
      `${top1.name} took the attack apart with a ${top1.runs}-ball blitz (${top1.runs} off ${top1.balls}, SR ${Math.round(sr1)}).`,
    );
  } else if (top1.runs >= 60) {
    lines.push(`${top1.name} anchored the innings with a composed ${top1.runs} off ${top1.balls}.`);
  } else {
    lines.push(
      `${top1.name} top-scored with ${top1.runs} off ${top1.balls} but never fully broke the shackles.`,
    );
  }
  ...
  if (firstBatting.partnership.runs >= 100) {
    lines.push(
      `A ${firstBatting.partnership.runs}-run stand between ${firstBatting.partnership.names[0]} and ${firstBatting.partnership.names[1]} formed the spine of the total.`,
    );
  } else if (firstBatting.partnership.runs >= 60) {
    lines.push(
      `${firstBatting.partnership.names[0]} and ${firstBatting.partnership.names[1]} added a valuable ${firstBatting.partnership.runs} to steady things.`,
    );
  }
  ...
  if (firstBatting.wickets >= 8) {
    lines.push(
      `The innings folded late — ${firstBatting.wickets} down for ${firstBatting.runs}, the tail giving in under pressure.`,
    );
  } else if (firstBatting.wickets <= 3 && firstBatting.overs >= maxOvers - 1) {
    lines.push(
      `${firstBatting.teamName} finished strongly — just ${firstBatting.wickets} down at the close, a platform they will fancy defending.`,
    );
  }
  ...
  if (b1.wickets >= 5) {
    lines.push(
      `${b1.name} ripped through with a five-for — ${b1.wickets}/${b1.runs} off ${b1.overs}, the spell of the match so far.`,
    );
  } else if (b1.wickets >= 3) {
    lines.push(
      `${b1.name} kept probing and picked up ${b1.wickets}/${b1.runs} — a genuine pressure spell.`,
    );
  ...
  // Break: target
  lines.push(
    `Innings break — ${firstBatting.teamName} ${firstBatting.runs}/${firstBatting.wickets}. ${secondBatting.teamName} need ${target} to win ${format === "T20" ? "at over eight an over" : ""}.`,
  );
  ...
  if (chaseWon) {
    ...
    if (secondBatting.topScorer.runs >= 80 && secondBatting.topScorer === secondBatting.topScorer) {
      lines.push(
        `${secondBatting.topScorer.name} led the chase with ${secondBatting.topScorer.runs} off ${secondBatting.topScorer.balls}, unflappable under lights.`,
      );
    }
    if (ballsRemaining <= 3 && wktsInHand <= 2) {
      lines.push(
        `Down to the final over, nerves shredded — ${secondBatting.teamName} scraped home with ${ballsRemaining} to spare.`,
      );
      ...
      lines.push(
        `${secondBatting.teamName} cruised over the line — ${wktsInHand} wickets in hand and plenty of overs left.`,
      );
    } else {
      lines.push(
        `${secondBatting.teamName} rebuilt smartly after early wobbles and completed the chase.`,
      );
    }
  } else {
    ...
    if (wktsDown === 10) {
      lines.push(
        `${secondBatting.teamName} were bundled out ${shortBy} short — the collapse started with ${secondBatting.bestBowler.name === secondBatting.topScorer.name ? "the top order" : `${firstBatting.bestBowler.name}'s second spell`}.`,
      );
    } else if (shortBy <= 10) {
      lines.push(
        `Heartbreak — ${secondBatting.teamName} came agonisingly close, falling ${shortBy} runs short off the final over.`,
      );
    } else {
      lines.push(
        `${secondBatting.teamName} never quite kept up with the required rate and finished ${shortBy} adrift.`,
      );
    }
  }
  ...
  // Player of the match
  lines.push(
    `${r.playerOfMatch} took Player of the Match honours in a ${r.weWon ? "famous win for" : "tough day for"} ${r.ourName}.`,
  );

  // Occasional flavour extras
  if (maybe(rng, 0.25))
    lines.push(
      `A brief rain interruption threatened to derail the innings, but the covers came off in time.`,
    );
  if (maybe(rng, 0.15))
    lines.push(`A brilliant piece of fielding in the deep swung momentum at a critical moment.`);
  ...
  if (r.result === "DRAW") {
    lines.push(
      sessionLine(
        4,
        "Afternoon",
        `A batting side happy to survive, a bowling side losing patience — the draw looked inevitable.`,
      ),
    );
    lines.push(
      sessionLine(
        5,
        "Final Session",
        `Handshakes all round. A ${pick(["stubborn", "attritional", "weather-hit"], rng)} draw ends the Test.`,
      ),
    );
    ...
    lines.push(
      sessionLine(
        4,
        "Evening",
        `${r.ourName} smelled blood — the field crept in and every ball felt like a moment.`,
      ),
    );
    lines.push(
      sessionLine(
        5,
        "Final Session",
        `${r.ourName} sealed it — ${r.marginText}. ${r.playerOfMatch} the deserved Player of the Match.`,
      ),
    );
  } else {
    lines.push(
      sessionLine(
        4,
        "Evening",
        `${r.oppName} kept squeezing — ${r.ourName} batters running out of ideas.`,
      ),
    );
    lines.push(
      sessionLine(
        5,
        "Final Session",
        `${r.oppName} closed it out — ${r.marginText}. A tough lesson for ${r.ourName}.`,
      ),
    );
  }
