...
export function overall(p: Player): number {
  const s = p.stats;
  const battingWeight = p.role === "Batsman" || p.role === "Wicketkeeper" ? 0.55
    : p.role === "AllRounder" ? 0.35 : 0.15;
  const bowlingWeight = p.role === "PaceBowler" || p.role === "SpinBowler" ? 0.55
    : p.role === "AllRounder" ? 0.35 : 0.10;
  const other = 1 - battingWeight - bowlingWeight;
  const utility = (s.fielding * 0.35 + s.pressure * 0.25 + s.consistency * 0.20 + s.fitness * 0.10 + s.leadership * 0.10);
...
export function computeTeamRating(players: Player[]): TeamRating {
  ...
  const batters = players.filter(p => p.role === "Batsman" || p.role === "Wicketkeeper" || p.role === "AllRounder");
  const bowlers = players.filter(p => p.role === "PaceBowler" || p.role === "SpinBowler" || p.role === "AllRounder");
